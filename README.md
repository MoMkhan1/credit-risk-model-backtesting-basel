...
ROC-AUC = 0.616
Indicates moderate predictive power — the model performs better than random classification but leaves room for improvement.
...
Large difference between expected and stressed losses (1,478 vs 17,502,970) demonstrates how adverse scenarios can dramatically increase portfolio risk.
...
All techniques align with Basel regulatory frameworks for credit risk quantification.

Research & Industry Relevance

This project is fully Dockerized and can run in a production-ready environment. It also supports GitHub Actions CI/CD for automated testing and deployment.
...
docker build -t credit-risk-pipeline .
# Run the full pipeline:
docker run --rm credit-risk-pipeline python scripts/credit_risk_pipeline.py

CI/CD Automation via GitHub Actions:

Automatically builds and tests the Docker image on push or pull request to main.
Executes the pipeline inside Docker to ensure reproducibility.
Uploads results, models, and visualizations as workflow artifacts.
Ensures code and results are version-controlled and production-ready.

Author

Mohamed Zaman
PhD Researcher — Quantitative Finance & Machine Learning
